...
class AdminProvider extends ChangeNotifier {
  ...
  List<MapEntry<String, int>> materiasCuelloDeBotella =
  ...
  Future<void> cargarEstadisticasDashboard() async {
    ...
    try {
      ...
      final snapshotTutorias = await _baseDeDatosOperativa
          .collection('tutorias')
          .get();
      ...
      for (var documento in snapshotTutorias.docs) {
        ...
        if (data['estadoDeLaSolicitud'] == 'finalizada') {
          ...
          if (data['fecha_creacion_solicitud'] != null) {
            DateTime? fecha = DateTime.tryParse(
              data['fecha_creacion_solicitud'],
            );
            if (fecha != null) {
              int weekOfYear = _getWeekOfYear(fecha);
              horasPorSemana[weekOfYear] =
                  (horasPorSemana[weekOfYear] ?? 0) + horas;
            }
          }

          final tutorNombre = data['nombre_tutor'] ?? 'Tutor Desconocido';
          horasPorTutor[tutorNombre] =
              (horasPorTutor[tutorNombre] ?? 0) + horas;
          ...
          horasPorAsignaturaTreemap[llaveTreemap] =
              (horasPorAsignaturaTreemap[llaveTreemap] ?? 0) + horas;

          if (listaAlumnos != null) {
            for (var uidAlumno in listaAlumnos) {
              horasPorEstudiante[uidAlumno.toString()] =
                  (horasPorEstudiante[uidAlumno.toString()] ?? 0) + horas;
            }
          }
        }
      }
      ...
      Map<String, Map<String, int>> clasesPorAlumnoYtutor =
          {}; // tutorUid -> { alumnoUid -> count }
      ...
      for (var doc in snapshotTutorias.docs) {
        ...
        // SLA Promedio
        if (data['fecha_creacion_solicitud'] != null &&
            data['fecha_aceptacion_solicitud'] != null) {
          final creacion = DateTime.tryParse(data['fecha_creacion_solicitud']);
          final aceptacion = DateTime.tryParse(
            data['fecha_aceptacion_solicitud'],
          );
          if (creacion != null &&
              aceptacion != null &&
              aceptacion.isAfter(creacion)) {
        ...
        if (estado == 'solicitada' && materia != null) {
          conteoMateriasSolicitadas[materia] =
              (conteoMateriasSolicitadas[materia] ?? 0) + 1;
        }

        // Conteo para cancelaciones
        if (tutorId != null && tutorId.isNotEmpty && estado != 'solicitada') {
          totalClasesProgramadasPorTutor[tutorId] =
              (totalClasesProgramadasPorTutor[tutorId] ?? 0) + 1;
        }
        ...
        if (estado == 'finalizada') {
          ...
          if (tutorId != null && tutorId.isNotEmpty) {
            ...
            final asistencia =
                data['registro_asistencia'] as Map<String, dynamic>? ?? {};
            for (var entry in asistencia.entries) {
              if (entry.value == true) {
                // Solo cuenta si realmente asistió
                clasesPorAlumnoYtutor[tutorId]![entry.key] =
                    (clasesPorAlumnoYtutor[tutorId]![entry.key] ?? 0) + 1;
              }
            }
          }
      ...
      if (totalAsistenciasEsperadas > 0) {
        indiceDesercionGlobal =
            (totalInasistencias / totalAsistenciasEsperadas) * 100.0;
      } else {
      ...
      final quejasSnapshot = await _baseDeDatosOperativa
          .collection('quejas')
          .get();
      Map<String, int> cancelacionesTardiasTutor = {};
      for (var q in quejasSnapshot.docs) {
        final d = q.data();
        if (d['motivo_sistema'] != null &&
            d['motivo_sistema'].toString().contains('Menos de 12h')) {
          String tId = d['tutorId'] ?? '';
          if (tId.isNotEmpty) {
            cancelacionesTardiasTutor[tId] =
                (cancelacionesTardiasTutor[tId] ?? 0) + 1;
          }
        }
      }
      ...
      final usuariosSnapshot = await _baseDeDatosOperativa
          .collection('usuarios')
          .get();
