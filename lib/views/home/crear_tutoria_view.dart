  ...
  Future<void> _confirmarYSubirTutoria(UsuarioModel usuarioCreador) async {
    ...
    if (_fechaSeleccionada == null || _horaSeleccionada == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Por favor, selecciona tanto la fecha como la hora sugerida para la tutoría.'),
    ...
    final String identificadorUnico = DateTime.now().millisecondsSinceEpoch.toString();
    ...
    final TutoriaModel miModelo = TutoriaModel(
      ...
      identificadorDelTutor: '', // Se inicializa vacío a la espera de que un profesor del sistema acepte el encargo
      listaDeEstudiantesInscritos: [usuarioCreador.identificadorUnico], // El autor solicitante ocupa automáticamente la primera plaza
    ...
    final operacionExitosa = await proveedorTutorias.crearAperturaDeNuevaTutoria(planoFormateadoDelExamen: miModelo);

    if (mounted) {
      if (operacionExitosa) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('La solicitud de tutoría ha sido publicada exitosamente en el foro mundial.'),
        ...
        body: Center(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.shield_outlined, size: 72, color: Colors.orangeAccent),
                ...
                builder: (context, elProveedorDeTutorias, elNinoNoUsado) {
                  return FilledButton(
                    ...
                    onPressed: elProveedorDeTutorias.estaCargandoPeticionEnNube
                        ? null
                        : () => _confirmarYSubirTutoria(usuarioActualDelDispositivo),
