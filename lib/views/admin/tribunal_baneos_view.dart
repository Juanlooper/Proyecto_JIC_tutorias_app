  ...
  Future<void> _perdonarExcusa(BuildContext context, String docId) async {
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Excusa aceptada. No se aplicó strike.'),
            backgroundColor: AppTheme.primarioVerde,
          ),
        );
      }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error al perdonar.'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _penalizarExcusa(
    ...
    try {
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        final reporteRef = FirebaseFirestore.instance
            .collection('reportes_tribunal')
            .doc(reporteId);
        ...
        if (usuarioSnap.exists) {
          final strikesActuales =
              usuarioSnap.data()?['strikes_inasistencia'] ?? 0;
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Strike aplicado exitosamente.'),
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error al penalizar.'),
            backgroundColor: Colors.red,
          ),
        );
      }
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(
                ...
                text: "Excusas Pendientes",
              ),
              Tab(icon: Icon(Icons.block), text: "Estudiantes Baneados"),
            ],
          ),
        ),
        body: TabBarView(children: [_buildTabExcusas(), _buildTabBaneados()]),
      ),
    );
  }
                      ...
                      children: [
                        ...
                        Expanded(
                          child: FilledButton.icon(
                            ...
                            onPressed: () =>
                                _penalizarExcusa(context, doc.id, alumnoId),
