...
import 'crear_clase_propia_view.dart';
...
class _DashboardTutorViewState extends State<DashboardTutorView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final idActual = context
          .read<AutenticacionProvider>()
          .usuarioActual
          ?.identificadorUnico;
      if (idActual != null) {
        context.read<TutoriasProvider>().cargarTutoriasSuscritasDelUsuario(
          idActual,
        );
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return const Center(
            child: Text("Error conectando con la bolsa de solicitudes."),
          );
        }
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return _construirLista(
            [],
            0,
          ); // Re-utilizamos el constructor de estado vacío natural
        }

        final documentosNube = snapshot.data!.docs;
        final filtrados = documentosNube
            .map((doc) {
              return TutoriaModel.fromMap(doc.data() as Map<String, dynamic>);
            })
            .where((t) => t.identificadorDelTutor.isEmpty)
            .toList(); // Filtro Restrictivo #2
  ...
  Widget _construirTarjeta(TutoriaModel tutoria, int tipo) {
    final fecha =
        '${tutoria.fechaHoraSugerida.day.toString().padLeft(2, '0')}/${tutoria.fechaHoraSugerida.month.toString().padLeft(2, '0')} - ${tutoria.fechaHoraSugerida.hour.toString().padLeft(2, '0')}:${tutoria.fechaHoraSugerida.minute.toString().padLeft(2, '0')} Hrs';
          ...
          children: [
            ...
            Align(
              ...
              child: Chip(
                ...
                backgroundColor:
                    AppTheme.primarioVerde, // O un color vibrante como Naranja
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => AceptarSolicitudView(tutoria: tutoria),
                      ),
                    );
                  },
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => DetalleClaseView(tutoria: tutoria),
                      ),
                    );
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                        AppTheme.primarioVerde, // Diferenciador visual
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12),
                    ),
                    padding: const EdgeInsets.symmetric(vertical: 14),
  ...
  Widget _construirLista(List<TutoriaModel> lista, int tipo) {
    if (lista.isEmpty) {
      ...
      if (tipo == 0) {
        ...
        msj = "No tienes clases archivadas en el historial";
      }
        ...
        body: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance
              ...
              .where(
                ...
                isEqualTo: usuario.identificadorUnico,
              )
              .snapshots(),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(
                child: CircularProgressIndicator(color: AppTheme.primarioVerde),
              );
            }
            if (snapshot.hasError) {
              return const Center(
                child: Text("Error obteniendo datos del panel en la nube."),
              );
            }

            final docsNube = snapshot.data?.docs ?? [];
            final misTutoriasDictando = docsNube
                .map(
                  (doc) =>
            ...
            final pendientes = misTutoriasDictando.where((t) {
              return t.estadoDeLaSolicitud != 'finalizada' &&
                  ...
                  t.estadoDeLaSolicitud != 'sugerida_directa' &&
                  t.estadoDeLaSolicitud != 'rechazada_directa';
            }).toList();
