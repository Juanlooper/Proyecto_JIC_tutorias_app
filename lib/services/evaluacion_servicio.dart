...
class EvaluacionServicio {
  ...
  final FirebaseFirestore _bovedaDocumentalAuditoria =
  ...
  Future<String> registrarEvaluacionCompleta({
    ...
    required int
    calificacionDeLaClase, // Escala de 1 a 5 (¿Qué tan buena fue la metodología o contenido?)
    required int
    calificacionDelTutor, // Escala de 1 a 5 (¿Qué tan bueno, amable o puntual fue el humano involucrado?)
    ...
    try {
      ...
      await _bovedaDocumentalAuditoria.collection('evaluaciones').add({
        ...
        'comentarioAdicional':
            comentarioEscritoOpcional ??
            '', // Si el alumno fue tímido y no quiso escribir, va vacío
  ...
  Future<String> reportarIncidenteOQueja({
    ...
    try {
      await _bovedaDocumentalAuditoria.collection('quejas').add({
        ...
        'estadoDeInvestigacion':
            'pendiente_revision', // Identificador vital para el panel de Maiky.
  ...
  Future<double> obtenerPromedioDeTutor({
    ...
    try {
      ...
      QuerySnapshot lecturaDelHistorialDelTutor =
          await _bovedaDocumentalAuditoria
              .collection('evaluaciones')
              .where(
                'identificadorDelTutor',
                isEqualTo: identificadorEspecificoDelTutor,
              )
              .get();
      ...
      for (var hojaDocumentalUnica in lecturaDelHistorialDelTutor.docs) {
        var informacionMapaDelBoleto =
            hojaDocumentalUnica.data() as Map<String, dynamic>;
        ...
        int calificacionHaciaProfesionalExtraida =
            informacionMapaDelBoleto['calificacionDelTutor'] ?? 5;

        sumatoriaTotalDeLasEstrellasGrapadas =
            sumatoriaTotalDeLasEstrellasGrapadas +
            calificacionHaciaProfesionalExtraida;
      }

      // PASO #4: Fórmula clásica del promedio: Total acumulado dividido por todo el volumen evaluativos ingresados.
      double promedioMateOficialAcumulado =
          sumatoriaTotalDeLasEstrellasGrapadas / cantidadCensadaDeVotantes;
